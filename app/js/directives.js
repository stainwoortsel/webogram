...
angular.module('myApp.directives', ['myApp.filters'])
    ...
    .directive('myMessageBody', function ($compile, AppPeersManager, AppChatsManager, AppUsersManager, AppMessagesManager, AppInlineBotsManager, RichTextProcessor) {
        ...
        function updateMessageText($scope, element, message) {
            if ((message.media && message.media.handleMessage) ||
                typeof message.message !== 'string' || !message.message.length) {
        ...
        function link($scope, element, attrs) {
            ...
            function onKeyDown(e) {
                ...
                if (searchFocused && e.keyCode == 13 && !Config.Navigator.mobile) { // Enter
                    ...
                    if (currentSelected && !$(currentSelected).hasClass('disabled')) {
    ...
    .directive('mySidepanel', function ($window, $timeout, $rootScope, $transition) {
        return {
            link: link
        }

        function link($scope, element, attrs) {
            var sidepanelWrap = $('.im_sidepanel_wrap', element)
            var sidepanelEl = $('.im_sidepanel', element)[0]
            var sideScrollableWrap = $('.im_sidepanel_scrollable_wrap', element)
            var sideScrollable = $('.im_sidepanel_scrollable', element)[0]

            onContentLoaded(function () {
                sideScrollableWrap.scrollTop = sideScrollableWrap.scrollHeight
            })

            $(sidepanelWrap).nanoScroller({preventPageScrolling: true, tabIndex: -1, iOSNativeScrolling: true})

            var updateScroller = function (delay) {
                // console.trace('scroller update', delay)
                $timeout(function () {
                    if (!$(sideScrollableWrap).hasClass('im_sidepanel_to_bottom')) {
                        $(sidepanelWrap).nanoScroller()
                    }
                }, delay || 0)
            }
            ...
            function updateSizes(heightOnly) {
                ...
                var padding = 15;
                var appH = $($window).height() - (headWrap ? headWrap.offsetHeight : 48) - footerHeight
                var panelH = Math.floor(appH / 2) // - padding * 2
            ...
            $scope.$on('sidepanel_flush', function () {
                updateSizes();
                var $sidePabelWrap = $('.im_sidepanel_col_wrap');
                if (!$sidePabelWrap.hasClass('im_sidepanel_loaded')) {
                    $sidePabelWrap.addClass('im_sidepanel_loaded');
                }
            })
    ...
    .directive('myLoadRound', function (AppMessagesManager, AppDocsManager, $timeout) {
        ...
        function readVideoMessage($scope) {
            if ($scope.message && !$scope.message.pFlags.out &&
        ...
        function link($scope, element, attrs) {
            ...
            if (attrs.watch) {
                $scope.$on('user_update', function (e, updUserID) {
                    if (peerID == updUserID) {
                        peer = AppPeersManager.getPeer(peerID)
                        if (!angular.equals(peer.photo && peer.photo.photo_small, peerPhoto) || !peerPhoto) {
                            updatePeerPhoto()
                        }
                    }
                })
