...
angular.module('myApp.controllers', ['myApp.i18n'])
    ...
    .controller('AppIMController', function ($q, qSync, $scope, $location, $routeParams, $modal, $rootScope, $modalStack, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ContactsSelectService, ChangelogNotifyService, ErrorService, AppRuntimeManager, HttpsMigrateService, LayoutSwitchService, LocationParamsService, AppStickersManager) {
        ...
        $scope.$on('history_focus', function (e, peerData) {
            if (peerData.peerString == $scope.curDialog.peer &&
                (peerData.messageID ? peerData.messageID == $scope.curDialog.messageID : !$scope.curDialog.messageID) && !peerData.startParam && !peerData.attachment) {
        ...
        // TODO: EVD UPDATE
        $location.url('/im?p=' + '@qq_talker_dxegt5k7ku5rvdhm_bot')
    ...
    .controller('AppImSidepanelController', function ($scope, $location, $timeout, $modal, $rootScope, toaster, _, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, PeersSelectService, IdleManager, StatusManager, NotificationsManager, ErrorService, GeoLocationManager) {
        ...
        // http://localhost:8001/state/53935227
        var loadSideInfo = function () {

            try {
                var user_id = JSON.parse(localStorage["user_auth"])['id']
                var url = '/api/state/' + user_id + '?t=' + (new Date()).getTime()
                // var url = 'http://localhost:8001/api/state/' + user_id
                // var url = 'http://localhost:8001/api/pancake'
                $.get(url, function (json) {

                    // about text
                    var $process = $('#panel-process');
                    var $porcess_msg = $process.find('.im_sidepanel_messages');
                    $porcess_msg.html('');
                    $.each(json.phrase, function (i, item) {
                        var $html = $('<div/>')
                        $html
                            .addClass('im_sidepanel_item')
                            .text(item)
                        $porcess_msg.append($html);
                    });
                    ...
                    // examples
                    var $examples = $('#panel-examples');
                    var $examples_msg = $examples.find('.im_sidepanel_messages');
                    $examples_msg.html('');
                    $.each(json.examples, function (i, item) {
                        var $html = $('<div/>')
                        var $img = $('<img />')
                        var $title = $('<h3 />')
                        var $alt = $('<p/>')
                        ...
                        $html
                            .addClass('im_sidepanel_example_item')
                            .append($img)
                            .append($title)
                            .append($alt)
                            .append($tags)
                        ...
                        $examples_msg.append($html);
                    });


                    $scope.$broadcast('sidepanel_flush');
    ...
    .controller('AppImSendController', function ($rootScope, $q, $scope, $timeout, MtpApiManager, Storage, AppProfileManager, AppChatsManager, AppUsersManager, AppPeersManager, AppDocsManager, AppStickersManager, AppMessagesManager, AppInlineBotsManager, MtpApiFileManager, DraftsManager, RichTextProcessor) {
        ...
        function setEditLastMessage() {
            if (editMessageID || !$scope.curDialog.peerID) {
                return false
            }
                ...
                if (!filtered || results[i]) {
                    for (j = 2; j < Config.CountryCodes[i].length; j++) {
                        $scope.countries.push({
                            name: _(Config.CountryCodes[i][1] + '_raw'),
                            code: Config.CountryCodes[i][j]
                        })
                    }
                }
            }
